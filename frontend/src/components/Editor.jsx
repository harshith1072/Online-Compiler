...
import MonacoEditor from "@monaco-editor/react";
import api from "./api"; 
import axios from "axios"; // Import axios directly for the compiler calls
import "../Styles/Editor.css";
import TestCaseForm from "./TestCaseForm";
...
        try {
            // Use axios with the compiler URL
//             const { data } = await axios.post(`${config.COMPILER_URL}/ai-review`, payload);
//             const { data } = await axios.post(`https://compiler-kyus.onrender.com/ai-review`, payload);
...
        try {
            // Use axios with the compiler URL
//             const { data } = await axios.post(`${config.COMPILER_URL}/run`, payload);
//             const { data } = await axios.post(`https://compiler-kyus.onrender.com/run`, payload);
...
        try {
            // Step 1: Run code on the compiler
//             const { data } = await axios.post(`${config.COMPILER_URL}/run`, payload);
//             const { data } = await axios.post(`https://compiler-kyus.onrender.com /run`, payload);
            const { data } = await axios.post(`https://online-compiler-076b.onrender.com/run`, payload);
...
            // Step 3: Send submission to the backend
...
    const [customInput, setCustomInput] = useState("");
    const [customOutput, setCustomOutput] = useState("");
    const [verdict, setVerdict] = useState(""); // Add state for verdict
...
            // Use axios with the compiler URL
//             const { data } = await axios.post(
//                 `${config.COMPILER_URL}/custom`,
//                 payload
//             );
  const { data } = await axios.post(
                `https://online-compiler-076b.onrender.com/custom`,
                payload
            );
            console.log(data);
            setOutput1(data.output);
